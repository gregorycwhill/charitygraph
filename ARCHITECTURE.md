...
After the bounded knowledge-primitives tranche, the next implementation task is the first private reality slice described by the canonical Data [IMPLEMENTATION_PLAN.md](https://github.com/gregorycwhill/charitygraph-data/blob/main/IMPLEMENTATION_PLAN.md): an approximately ten-charity identity/program/classification pipeline using the implemented contracts, SQLite control plane, evidence storage and bounded model judgment. It remains private and review-only. Do not alter public schemas, rebuild a release, move archive evidence, or make unapproved provider/model calls.
