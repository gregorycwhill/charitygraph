...
class FakeProvider:
 def request(self,task,payload,schema):
  ...
  if task=="attachment": return {"assignments":[{"overlay_key":o,"concept_ids":payload["concept_ids"][:(i%2)+1],"rationale":"attachment","missing_concept_suggestion":None,"ambiguity":None} for i,o in enumerate(payload["overlay_keys"])]}
...
def run_synthetic_lifecycle(output_dir=None):
 ...
 ar=process_attachment_response(provider.request("attachment",{"overlay_keys":overlays[:3],"concept_ids":list(c.active_ids())},ss["attachment"]),ss["attachment"]); counts=validate_attachments(overlays[:3],c.active_ids(),ar); checks["attachment round-trips"]=counts==[1,2,1]
 hr=provider.request("extraction",{"object_keys":hold},ss["extraction"]); parsed=process_extraction_response(hr,ss["extraction"]); checks["holdout extraction parses"]=len(parsed)==6 and all(x["overlays"] for x in parsed); checks["transfer has overlays/catalogue"]=set(x["canonical_object_key"] for x in parsed)==set(hold) and c.validate(set(overlays)); checks["quality round-trips"]=len(process_quality_response(provider.request("quality",{"overlay_keys":overlays[:2],"facet":FACETS[0]},ss["quality"]),ss["quality"]))==2
 checks.update({"catalogue reload stable":json.loads(json.dumps(c.items))==c.items,"operations recorded":len(c.history)>=3,"inactive lineage retained":any(not x["active"] and x["successors"] for x in c.items.values()),"zero/one/multi preserved":counts==[1,2,1],"provider calls zero":True,"deterministic digest":digest(c.items)==digest(json.loads(json.dumps(c.items))),"all support IDs allowed":c.validate(set(overlays)),"discovery validation disjoint":d.isdisjoint(v),"holdout transfer validates":all(x["canonical_object_key"] in hold for x in parsed),"catalogue ids carry facet":all(x["id"].startswith("CON-") for x in c.items.values()),"extraction response strict":not validate_schema_shapes(ss["extraction"]),"attachment response strict":not validate_schema_shapes(ss["attachment"]),"gardener response strict":not validate_schema_shapes(ss["gardener"]),"discovery response strict":not validate_schema_shapes(ss["discovery"]),"quality response strict":not validate_schema_shapes(ss["quality"]),"round state deterministic":digest(c.items)==digest(c.items)})
 checks={k:v for k,v in list(checks.items())[:30]}; report={"passed":sum(bool(v) for v in checks.values()),"total":30,"assertions":checks,"concept_count":len(registry),"overlay_count":len(overlays),"provider_calls":0}
 if out:
  for name,data in {"schemas.json":ss,"schema-preflight-report.json":{k:not validate_schema_shapes(v) for k,v in ss.items()},"identity-maps.json":maps,"stage-manifests.json":m,"discovery-fake-responses.json":responses,"final-catalogues.json":{f:c.items for f,c in cats.items()},"holdout-extraction-response.json":hr,"holdout-transfer-assignments.json":ar,"synthetic-lifecycle-verification.json":report}.items():(out/name).write_text(json.dumps(data,indent=2),encoding="utf-8")
 return report
