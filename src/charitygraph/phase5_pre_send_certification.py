...
def certify_standard_execution_row(catalog: Any, row: Mapping[str, Any], *, mandate_id: str) -> dict[str, Any]:
    ...
    with catalog._authorization_connection() as conn:
        reservation = conn.execute("SELECT status,reserved_aud FROM execution_mandate_reservations WHERE mandate_id=? AND reservation_id=?", (mandate_id, row["mandate_reservation_id"])).fetchone()
    if reservation is None or reservation["status"] != "active" or Decimal(reservation["reserved_aud"]) < Decimal(str(row["hard_max_aud"])):
        raise ValueError("execution certification mandate reservation is missing or insufficient")
