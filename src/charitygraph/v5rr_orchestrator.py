...
PROMPTS={
 ...
 "attachment": """Assign each reviewed overlay to active concepts in the supplied catalogue. Return exactly one assignment row for every supplied overlay_key, preserving the supplied overlay_key exactly and including an empty concept_ids list when no assignment is warranted. Zero, one, or multiple distinct assignments are legitimate. Do not force-fit. Report missing-concept suggestions and ambiguities where warranted.""",
