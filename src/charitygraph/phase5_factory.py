...
class ReferenceFactory:
    ...
    def seed(self, now: datetime) -> tuple[dict[str, Any], ...]:
        ...
        return tasks
    def prepare_package(self, tasks: tuple[dict[str, Any], ...], *, now: datetime) -> tuple[str, FakeProviderReceipt | None]:
        ...
        physical_id="physical:"+package_id.split(":",1)[1]
        ...
        receipt=self.provider.execute(semantic[0],delivery_mode=self.delivery_mode)
    ...
    def finalise_package_children(self, tasks: tuple[dict[str, Any], ...], *, package_id: str, receipt: FakeProviderReceipt | None, now: datetime) -> int:
        ...
        for task in tasks:
            ...
            if not self.catalog.claim_task(task["record_id"],owner=owner,lease_expires_at=now+timedelta(hours=1),now=now): continue
        ...
        if receipt is not None:
            reservation_id=deterministic_id("reservation:", {"physical_attempt": package_id})
            actual={"cohort_id":self.cohort_id,"run_id":self.run_id,"task_run_id":package_id,"reservation_id":reservation_id,"entry_type":"actual","paid_output_category":"extraction","provider_cost":{"amount":"0.001000","currency":"USD"},"aud_cost":{"amount":"0.001000","currency":"AUD"},"usage":receipt.usage,"recorded_at":now,"pricing_snapshot_id":deterministic_id("pricing:",{"rehearsal":"phase5"}),"fx_snapshot_id":deterministic_id("fx:",{"rehearsal":"phase5"})}
    ...
    def run(self, now: datetime) -> int:
        ...
        for package in self.plan.physical_packages(delivery_mode=self.delivery_mode):
            ...
            package_id,receipt=self.prepare_package(tasks,now=now)
            completed += self.finalise_package_children(tasks,package_id=package_id,receipt=receipt,now=now)
        return completed
