...
def _prompt() -> str:
    # This is the supplied frozen experiment prompt, kept verbatim in the
    # private harness rather than assembled from mutable product prompts.
    return Path(r"C:\Users\grego\.codex\attachments\33c22785-dd6d-470a-974e-9e535c6b8c07\pasted-text.txt").read_text(encoding="utf-8")
...
def _estimate(packet_bytes: bytes, prompt: str) -> tuple[int, Any]:
    ...
    # Two calls, each with one permitted transport retry: conservative exposure.
    projected = (2 * (tokens * 0.20 + MAX_TOKENS * 1.20) * 2) / 1_000_000
...
def run(output_root: Path | None = None) -> int:
    ...
    if projected_usd > SPEND_CAP_USD:
        raise RuntimeError(f"projected experiment spend exceeds USD {SPEND_CAP_USD:.2f}: {projected_usd:.6f}")
    ...
    for model in MODEL_NAMES:
        row: dict[str, Any] = {"model": model, "packet_sha256": hashlib.sha256(packet_bytes).hexdigest(), "projected_exposure_usd": projected_usd}
        try:
            ...
            try:
                parsed = json.loads(response.output_text); validated = validate_output(parsed, packet); row.update({"valid": True, "output": validated.model_dump(mode="json")})
            except Exception as exc:
                row.update({"valid": False, "validation_error": type(exc).__name__})
        except Exception as exc:
            row.update({"valid": False, "error": type(exc).__name__, "transport_requests": getattr(exc, "attempts_made", 0), "cost_usd": "0"})
