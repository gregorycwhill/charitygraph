...
def main():
 ...
 inv={(x['abn'],x['source_family']):x for x in read(clean/'corrected-pre-run-source-inventory.json')['cells']}; web={x.get('abn'):x for x in read(clean/'blocked-run-website-admissibility-ledger.json')['website_events'] if x.get('abn')}; blocked={x['abn']:x['coverage'] for x in read(root/'after-source-coverage-matrix.json')}
 rows=[]; hashes=[]
 for p in sorted((root/'corpora').glob('*.json')):
  ...
  for mraw in old['material_members']:
   ...
   if not keep:
    state={'unavailable':'attempted_unavailable','failed':'access_failed'}.get(state,state)
    m=gap(m,f,state,'excluded_or_unresolved_clean_lineage')
   else: state='acquired_available'
