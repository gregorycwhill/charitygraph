...
def main() -> int:
 ...
 factory=ReferenceFactory(catalog,plan,cohort_id=cohort,run_id=run); factory.seed(now); completed=factory.run(now); noop=factory.run(now); catalog.transition_run(run,'succeeded',now=now)
