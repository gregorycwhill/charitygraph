...
def main():
    ...
    for key,items in groups.items():
        packet=[{'atom_index':i,'proposition':a['raw_compact_atom']['proposition'],'evidence_items':[{'evidence_index':j,'canonical_ref':f"{e['source']}:{e['locator']}",'exact_excerpt':next((x.get('text','') for x in a.get('packet_evidence',[]) if x.get('locator')==e['locator']), '')} for j,e in enumerate(a['raw_compact_atom']['evidence'])]} for i,a in enumerate(items)]
        ...
        parsed=ScopeResolutionOutput.model_validate(json.loads(resp.output_text)); valid=[]; invalid=0
        for d in parsed.decisions:
            try: valid.append(ScopeResolutionOutput.__fields__ and d.model_dump(mode='json'))
            except Exception: invalid += 1
        out.append({'producer_task_id':key,'target':items[0]['target'],'status':resp.status,'decisions':valid,'invalid_decisions':invalid, 'input_tokens':usage.input_tokens,'output_tokens':usage.output_tokens,'cost_usd':str(cost),'latency_seconds':round(time.perf_counter()-started,3)})
