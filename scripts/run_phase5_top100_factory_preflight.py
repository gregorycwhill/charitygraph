...
def main() -> int:
    ...
    write_json(out / "phase5-planning-family-catalogue.json", [item.model_dump(mode="json") for item in policies])
    ...
    write_json(out / "unresolved-block-register.json", {"rank62": blocked, "policy_unresolved_families": [item.family_id for item in policies if item.maturity == "unresolved"]})
    ...
    readiness = baseline_readiness(coverage)
    write_json(out / "next-wave-readiness.json", readiness)
    write_json(out / "preflight-summary.json", {"build_id": BUILD_ID, "north_star_sections": SECTIONS, "cohort_manifest_sha256": COHORT_HASH, "identity_map_sha256": IDENTITY_MAP_HASH, "canonical_claim_family_ids": [item.family_id for item in canonical], "phase5_planning_family_ids": [item.family_id for item in policies], "phase5_planning_family_count": len(policies), "section_13_was_missing": "scheme-participation-v1" in [item.family_id for item in proposed], "planning_unit_count": len(matrix), "planning_state_counts": summarize(matrix), "future_work_by_method": workload_summary(matrix), "planned_logical_task_count": len(tasks), "physical_bundling_opportunities": bundle_opportunities, "source_coverage_summary": source_summary, "reuse_summary": reuse_summary, "historical_closeout": closeout, "interruption_safety": preflight_interruption_safety(), "next_wave_readiness": readiness, "subject_count_before": subject_count_before, "task_count_before": task_count_before, "subject_count_after": subject_count_before, "task_count_after": task_count_before, "provider_calls": 0, "source_acquisition": 0, "semantic_executions": 0, "canonical_observations_created": 0})
