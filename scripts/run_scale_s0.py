...
def main() -> int:
    ...
    with SQLiteCatalog(args.catalog).open() as catalog:
        summary = ScaleS0Executor(catalog=catalog, attempt_id=args.attempt_id, runtime_root=args.runtime_root).run(semantic=semantic, locator=locator, dry_run=args.preflight)
